...
def authenticate_image_client(predictortrain):
    # predictionendpoint
    ...
    if predictortrain == 'prediction':
        prediction_credentials = ApiKeyCredentials(in_headers={"Prediction-key": prediction_key})
        iclient = CustomVisionPredictionClient(ENDPOINT, prediction_credentials)
    else:
        trainer_credentials = ApiKeyCredentials(in_headers={"Training-key": training_key})
        iclient = CustomVisionTrainingClient(ENDPOINT, trainer_credentials)
    return iclient
...
#needs to be updated to a URL with training images
training_image_base_url = r"C:\Users\rober\OneDrive\Documents\Georgian Summer 2020\DataMining\SocialDataminingAssignmentFourRoutledge\GitRepo"
prediction_images = tweet_df['images']
trainer = authenticate_image_client('train')
#need to compile images to train on
#this code is based on the images being stored locally, but I'd bet we can find an image library online
#base_image_url =
#iterate over the images
# for image_num in range(1, len()):
...
predictor= authenticate_image_client('prediction')
#rewrite this as a loop that goes over very item in prediction_images
